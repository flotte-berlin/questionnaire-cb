      ...
      if (this.type !== Qst.TYPE_TEXT) {
        for (var i = 0; i < this.selections.length; ++i) {
          var data = {
            ...
            "selected": this.selected[this.selectedname[i]],
            name: this.name,
            value: this.selectedname[i],
            img: this.itemimages[i]
          };
          f(data);
        }
      }
    ...
    render: function() {
      ...
      switch (item.type) {
        ...
        case Qst.TYPE_DATETIME:
          ...
          datetimeinput.append(datetimetmpl({
            item: item
          }));
          var dateinput = "";
  ...
  var ActFormView = self.Qst.ActFormView = Backbone.View.extend({
    ...
    initialize: function(arg) {
      this.dialogView = new Qst.DialogView({
        el: $("div.qstnr-answersheet div.qstnr-dialog")
      });
    ...
    render: function() {
      ...
      _.each(this.model.itemlist(), function(item) {
        var formitem = new ActFormItemView({
          model: item,
          index: i
        });
        views.push(formitem);
      ...
      var data = {
        unique_name: this.model.unique_name,
        unique_email: this.model.unique_email,
        submit_text: this.model.submit_text
      };
    ...
    finished: function(bsuccess, data, jqXHR) {
      ...
      var hide = function(result) {
        ...
        if (this.model.keep_size) {
          ...
          this.hidediv.animate({
            'opacity': '0.0'
          }, 200, this.after_hide.bind(this));
        } else {
          this.hidediv.hide({
            duration: 500,
            complete: this.after_hide.bind(this)
          });
        }
    ...
    dosubmit: function(bsubmit) {
      if (bsubmit) {
        ...
        if (checked !== null) {
          ...
          this.inprogressView.ajaxcall2(this.model.save.bind(this.model), this.finished.bind(this), {}, {
  ...
  $(document).ready(function() {
    ...
    $.datepicker._updateDatepicker = function(inst) {
      ...
      if (inst.settings.useMonth === false) {
        inst.dpDiv.find(".ui-datepicker-month").css({
          "display": "none"
        });
      }
    };
